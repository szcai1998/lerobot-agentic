...
class SpatialGroundingPlan(BaseModel):
    ...
    Note on safety: `should_halt` represents a high-level supervisory software halt
    request (e.g. visual anomaly or foreign obstacle). True hardware emergency stop,
    watchdogs, joint limit enforcements, and safety PLCs must remain independent
    and operate at a lower level than this cognitive layer.
    """
    sub_goal: Literal["reach", "grasp", "lift", "transport", "place", "retreat", "recover"] = Field(
        description="Canonical active sub-task primitive: reach, grasp, lift, transport, place, retreat, or recover"
    )
    ...
    should_halt: bool = Field(default=False, description="Supervisory software halt request if anomaly or obstacle detected")
    decision_note: str | None = Field(default="", description="Short operational observation, e.g. 'target shifted', 'grasp verified', 'destination occluded'")
